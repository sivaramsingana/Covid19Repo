<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Covid19 Patient Tracker Portal</title>
</head>
<body>
<title>Covid19 Patient Tracker Portal</title>
</head>
<body>
<div style="width: 1280px;min-height: 85px;background-color: rgb(158,215,222);">
<h1 style="color:darkslateblue;">COVID19 PATIENT TRACKER</h1>
<p style="font-family: verdana;font-size: 12px;">Please use this page for tracking covid 19 patient details and their status.</p>
<img src="data:image/jpeg;base64,"
 style="width: 261px;float: right;height: 86px;position: relative;top: -88px;right: 49px;"/>
</div>
<div style="min-width:1280px ; min-height: 30px;color: olive; background-color:blanchedalmond; font-family: verdana;font-size: 13px;">
<div style="float: right;">
<a href="Index.jsp" style="width:5px">Home</a>
<a href="Register.jsp" style="padding: 7px;">Register Patient</a>
<a href="Updatestatus.jsp" style="padding: 7px;">Update Patient Status</a>
<a href="Search.jsp" style="padding: 7px;">Search patient</a>
<a href="Help.jsp" style="padding: 7px;">Help</a>
<a href="Contact.jsp" style="padding-right: 60px;">Contact</a>
</div>
</div>
<div>
<form action="ProfileUpdater" method="post">
<table>

<tr><td width="175px">Case Number:</td><td width="175px"><input type="number" name="casenum"></td></tr>
<tr><td width="175px">Name of the Patient:</td><td width="175px"><input type="text" name="pname"></td></tr>
<tr><td width="175px">Age:</td><td width="175px"><input type="number" name="age"></td></tr>
<tr><td width="175px">Aadhar Number:</td><td width="175px"><input type="text" name="Pid"></td></tr>
<tr><td width="175px">Phone Number:</td><td width="175px"><input type="number" name="phnumber"></td></tr>
</table>
<div>
Address
</div>
<table>
<tr><td width="175px">Street Number:</td><td width="175px"><input type="text" name="streetnum"></td></tr>
<tr><td width="175px">City:</td><td width="175px"><input type="text" name="city"></td></tr>
<tr><td width="175px">District:</td><td width="175px"><input type="text" name="district"></td></tr>
<tr><td width="175px">State:</td><td width="175px"><input type="text" name="state"></td></tr>
<tr><td width="175px">Country:</td><td width="175px"><input type="text" name="country"></td></tr>
</table>
<div>Other Details</div>
<table>
<tr><td width="175px">Date of Admission:</td><td width="175px"><input type="date" name="admitdate"></td></tr>
<tr><td width="175px">TestResult:</td><td width="175px"><select name="testresult"><option selected="selected">No</option><option>Yes</option></select></td></tr>
<tr><td width="175px">RecoveryStatus:</td><td width="175px"><select name="recoverstatus"><option selected="selected">No</option><option>Yes</option></select></td></tr>
<tr><td width="175px">Person Interacted:</td><td width="175px"><input type="text" name="picontact"></td></tr>
<tr><td width="175px">Parent Contaminated:</td><td width="175px"><input type="text" name="parentid"></td></tr>
<tr><td width="175px">Quarantine Status:</td><td width="175px"><select name="Qtnstatus"><option selected="selected">No</option><option>Yes</option></select></td></tr>
<tr><td width="175px">Quarantine Place:</td><td width="175px"><input type="text" name="place"></td></tr>
<tr><td width="175px">Health Issues</td><td width="175px"><input type="text" name="healthIssue"></td></tr>
<tr><td width="175px"></td><td width="175px"></td></tr>
<tr><td width="175px"></td><td width="175px"></td></tr>
<tr><td>
<input type="submit" value="Register"></td>
</tr>
</table>
</form>
</div>


</body>
</html>