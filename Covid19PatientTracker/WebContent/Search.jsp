<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    ...
    <%@page import="org.apache.poi.ss.usermodel.Row,com.covid.bean.PatientBean" %>
...
<tr bgcolor="00FF7F"><td>Search for a patient</td><td><input type="text" name="patientsearch"></td></tr>
<tr><td><input type="submit" name="Searchbtn" /></td></tr>
</table>
...
<%
		...
		{	
		list = (ArrayList<PatientBean>) request.getAttribute("resultarray");
		}
...
<th>Patient Name</th>
<th>Case Number</th>
<th>Patient Recovery Status</th>
<th>Patient Contacted Persons</th>
<th>Quarantine Status</th>
	...
	if(!"".equals(list)){
		int i=0,j=0;;
		for(PatientBean p: list)
				 {
                PatientBean actor = new PatientBean();
                actor = list.get(i);
...
<tbody>
            <tr>
            	<td><%= ++j %></td>
            	<td><%=actor.getPatientName() %></td>
                <td><%=actor.getCaseNumber()%></td>
                <td><%=actor.getRecoveryStatus()%></td>
                <td><%=actor.getPersonInteracted()%></td>
               	<td><%=actor.getQuarantineStatus()%></td>
            </tr>
            <%
            i++;
           		 }
